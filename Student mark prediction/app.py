...
def predict():
    ...
    # input and predicted value store in dataset then save in csv file
    dataset = pd.concat([dataset,pd.DataFrame({'Study Hours':input_features,'Predicted Output':[output]})],ignore_index = True)
    print(dataset)
    dataset.to_csv('smp_data_from_app.csv')
