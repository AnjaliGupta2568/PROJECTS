...
def generate_wordcloud(text):
    ...
    wordcloud = WordCloud(width=800, height=400, background_color='lightgreen').generate(english_text)
    fig ,ax =plt.subplots()
...
if translated_paragraph.strip():  #  Check if translated paragraph is not empty
       try:
           #Encoded text to UTF-8 to handle non-Unicode characters
...
if st.button("Translate and Read Aloud"):
    ...
    for target_language in target_languages:
          try:
              ...
              # Read translated paragraph aloud
              read_aloud(translated_paragraph, target_language)
                                                        
          except Exception as e:
              st.error(f"Error translating to {target_language}: {e}")