...
df = pd.read_csv(r"C:\AVSCODE\AI_code\language.csv")
df.dropna(inplace=True)
lang = df['name'].to_list()
langlist=tuple(lang)
langcode = df['iso'].to_list()

# create dictionary of language and 2 letter langcodes
...
def get_binary_file_downloadser_html(bin_file,file_label='File'):
    with open(bin_file,'rb') as f:
        data = f.read()
    bin_str = base64.b64encode(data).dacode()
...
if len(inputtext) > 0:
    try:
        ...
        # if speech support is availabel will render autio file
